  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-foreground">My Channels</h2>
        <button
          onClick={() => setShowForm(!showForm)}
          className="glass-button rounded-full px-3 py-1.5 text-xs font-medium text-white"
        >
      ...
      {showForm && (
        <div className="glass-card relative noise-overlay p-4 space-y-3">
          <input
            type="text"
          ...
          <button
            ...
            disabled={!name.trim()}
            className="w-full glass-button py-2 text-sm font-medium text-white disabled:opacity-50"
          >
            Create Channel
          </button>
        </div>
      ...
      <div className="space-y-2">
        {channels.map((ch) => (
          <div
            key={ch.id}
            className="flex items-center justify-between glass-card relative noise-overlay p-3"
          >
