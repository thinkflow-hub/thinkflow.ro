...
export default async function NewsArchivePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12">
      <h1 className="text-3xl font-montserrat-bold uppercase mb-2 text-foreground">News Archive</h1>
      ...
      <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-3">
        {dates.map((date) => (
          <Link
            ...
            className="glass-card relative noise-overlay px-4 py-3 text-sm font-medium text-foreground hover:text-accent transition-colors"
          >
            {date}
          </Link>
        ))}
